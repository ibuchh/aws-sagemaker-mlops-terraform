...
//         DOCKER_CONFIG= "${params.JENKINSHOME}"
    ...
    stages {
        stage("Checkout") {
            steps {
               checkout([$class: 'GitSCM', branches: [[name: '*/master']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[url: 'https://github.com/ibuchh/aws-sagemaker-mlops-terraform']]])
            }
        }
      ...
      stage("TrainStatus") {
            steps {
              script {
                    def response = sh """ 
                    TrainingJobStatus=`aws sagemaker describe-training-job --region us-east-1 --training-job-name \"${params.SAGEMAKER_TRAINING_JOB}-${env.BUILD_ID}\" | grep -Po \'"\'"TrainingJobStatus"\'"\\s*:\\s*"\\K([^"]*)\'`
